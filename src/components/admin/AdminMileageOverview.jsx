// src/components/admin/AdminMileageOverview.jsx - FIXED import paths and userProfile issue
...
const AdminMileageOverview = ({ coaches = [], clients = [], userProfile = null }) => {
  ...
  // ENHANCED Export to CSV with professional formatting
  const exportToCSV = () => {
    ...
    if (filterType === 'month') {
      ...
      reportPeriod = `${formatMileageDate(startDate)} to ${formatMileageDate(endDate)}`;
    } else {
    ...
    const totalMiles = filteredRecords.reduce((sum, record) => sum + record.mileage, 0);
    const totalClientMiles = filteredRecords.reduce((sum, record) => 
      sum + (record.transportedClients?.reduce((clientSum, tc) => clientSum + tc.mileage, 0) || 0), 0);
    const totalRecords = filteredRecords.length;
    const uniqueCoaches = [...new Set(filteredRecords.map(record => getCoachName(record.coachId)))];
    const totalClientTransports = filteredRecords.reduce((sum, record) => 
      sum + (record.transportedClients?.length || 0), 0);
    
    // Create enhanced CSV content
    const csvLines = [];
    
    // Header information
    csvLines.push('"ITG Mileage Report"');
    csvLines.push(`"Report Period:","${reportPeriod}"`);
    csvLines.push(`"Generated On:","${currentDate} at ${currentTime}"`);
    csvLines.push(`"Generated By:","${userProfile?.name || 'Program Administrator'}"`);
    csvLines.push(''); // Empty line
    
    // Summary section
    csvLines.push('"SUMMARY STATISTICS"');
    csvLines.push(`"Total Records:","${totalRecords}"`);
    csvLines.push(`"Total Coach Miles:","${totalMiles.toFixed(3)}"`);
    csvLines.push(`"Total Client Transportation Miles:","${totalClientMiles.toFixed(3)}"`);
    csvLines.push(`"Total Client Transports:","${totalClientTransports}"`);
    csvLines.push(`"Number of Coaches:","${uniqueCoaches.length}"`);
    csvLines.push(`"Average Miles per Record:","${totalRecords > 0 ? (totalMiles / totalRecords).toFixed(3) : '0.000'}"`);
    csvLines.push(''); // Empty line
    
    // Coach summary section
    if (uniqueCoaches.length > 0) {
      csvLines.push('"COACH SUMMARY"');
      csvLines.push('"Coach Name","Records","Total Miles","Client Transport Miles","Client Transports"');
      
      uniqueCoaches.forEach(coachName => {
        const coachRecords = filteredRecords.filter(record => getCoachName(record.coachId) === coachName);
        const coachMiles = coachRecords.reduce((sum, record) => sum + record.mileage, 0);
        const coachClientMiles = coachRecords.reduce((sum, record) => 
          sum + (record.transportedClients?.reduce((clientSum, tc) => clientSum + tc.mileage, 0) || 0), 0);
        const coachClientTransports = coachRecords.reduce((sum, record) => 
          sum + (record.transportedClients?.length || 0), 0);
        
        csvLines.push(`"${coachName}","${coachRecords.length}","${coachMiles.toFixed(3)}","${coachClientMiles.toFixed(3)}","${coachClientTransports}"`);
      });
      csvLines.push(''); // Empty line
    }
    
    // Detailed records section
    csvLines.push('"DETAILED MILEAGE RECORDS"');
    csvLines.push('"Date","Day of Week","Coach Name","Start Location","End Location","Business Purpose","Coach Miles","Transported Clients","Client Miles","Notes"');
    
    // Sort records by date (newest first)
    const sortedRecords = [...filteredRecords].sort((a, b) => new Date(b.date) - new Date(a.date));
    
    sortedRecords.forEach(record => {
      const date = formatMileageDate(record.date);
      const dayOfWeek = new Date(record.date + 'T12:00:00').toLocaleDateString('en-US', { weekday: 'long' });
      const coachName = getCoachName(record.coachId);
      const startLocation = record.startLocation.replace(/"/g, '""'); // Escape quotes
      const endLocation = record.endLocation.replace(/"/g, '""');
      const purpose = record.purpose.replace(/"/g, '""');
      const coachMiles = record.mileage.toFixed(3);
      
      // Format transported clients
      let transportedClientsText = '';
      let clientMilesText = '0.000';
      
      if (record.transportedClients && record.transportedClients.length > 0) {
        transportedClientsText = record.transportedClients
          .map(tc => `${tc.clientName} (${tc.mileage.toFixed(3)} mi)`)
          .join('; ');
        const totalClientMiles = record.transportedClients.reduce((sum, tc) => sum + tc.mileage, 0);
        clientMilesText = totalClientMiles.toFixed(3);
      }
      
      // Additional notes (could be expanded)
      const notes = record.transportedClients && record.transportedClients.length > 0 
        ? `${record.transportedClients.length} client(s) transported`
        : 'No clients transported';
      
      csvLines.push(`"${date}","${dayOfWeek}","${coachName}","${startLocation}","${endLocation}","${purpose}","${coachMiles}","${transportedClientsText}","${clientMilesText}","${notes}"`);
    });
    
    // Add footer
    csvLines.push(''); // Empty line
    csvLines.push('"End of Report"');
    csvLines.push(`"Total Records in Report:","${totalRecords}"`);
    csvLines.push(`"Report Generated:","${currentDate} ${currentTime}"`);
    ...
    const sanitizedPeriod = reportPeriod.replace(/[^a-zA-Z0-9]/g, '_');
    const timestamp = new Date().toISOString().split('T')[0]; // YYYY-MM-DD format
    a.download = `ITG_Mileage_Report_${sanitizedPeriod}_${timestamp}.csv`;
    ...
    alert(`✅ Enhanced mileage report exported successfully!\n\nReport: ${reportPeriod}\nRecords: ${totalRecords}\nTotal Miles: ${totalMiles.toFixed(3)}\nFile: ITG_Mileage_Report_${sanitizedPeriod}_${timestamp}.csv`);
  };
